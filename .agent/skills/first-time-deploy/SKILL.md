...
version: "1.0.0"
description: "Publish this project to GitHub Pages on git.soma.salesforce.com. For repeat deploys, just run `npm run deploy`. This skill covers first-time setup: configuring Pages and running the initial deploy. Use when the user asks about deploying, publishing, sharing a link with a PM or stakeholder, or setting up GitHub Pages."
---
...
Run the **repo-setup** skill first (`.agent/skills/repo-setup/SKILL.md`). It handles brew, `gh` CLI, authentication, repo creation, and initial push.
...
```bash
gh api repos/{owner}/{repo}/pages --hostname git.soma.salesforce.com --jq '.source'
```
...
```bash
gh api repos/{owner}/{repo}/pages --hostname git.soma.salesforce.com --jq '.html_url'
```
